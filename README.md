...
| :--- | :--- |
| `./run` / `./run dev` | Build both modules, launch the web control service on all network interfaces at port 8080. Add `--port 8090` to use a different port (8081 is taken by Flink's own Web UI, below). |
...
While a job is running, Flink's own Web UI is served from the embedded MiniCluster at
**http://localhost:8081** — the operator DAG, checkpoint history, backpressure, and
per-subtask metrics, none of which the dashboard above covers. It starts with the job and
stops with it, so the page is only live between **Start** and **Stop**.
